...
public class SparkJoinOperatorTest extends SparkOperatorTestBase {
    ...
    public void testExecution() {
        // Prepare test data.
        JavaRDD<Tuple2> inputStream0 = this.getSC().parallelize(Arrays.asList(
                new Tuple2<>(1, "b"), new Tuple2<>(1, "c"), new Tuple2<>(2, "d"), new Tuple2<>(3, "e")));
        JavaRDD<Tuple2> inputStream1 = this.getSC().parallelize(Arrays.asList(
        ...
        final List<Tuple2> result = ((JavaRDD<Tuple2>) outputStreams[0]).collect();
