...
(baseBuilder: DataQuantaBuilder[_, Record])
  extends RecordDataQuantaBuilder[This] {
  ...
  override def withOutputType(outputType: DataSetType[Record]): This = {
    baseBuilder.withOutputType(outputType)
    this.asInstanceOf[This]
  }

  override def withOutputClass(cls: Class[Record]): This = {
  ...
  override protected[api] def dataQuanta(): DataQuanta[Record] = baseBuilder.dataQuanta()
}
