...
expected SQL reached Trino. Most tests run a Wayang plan end to end; the join
test directly validates and executes the SQL clause generated by
`TrinoJoinOperator`.
...
tables afterward. It does not require `scripts/init.sql`.
...
| `join` | Trino join operator generates and executes a valid join clause |
