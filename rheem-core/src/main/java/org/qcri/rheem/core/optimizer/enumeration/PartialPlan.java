...
public class PartialPlan {
    ...
    private boolean isStartOperator(ExecutionOperator executionOperator) {
        for (InputSlot<?> inputSlot : executionOperator.getAllInputs()) {
            final InputSlot<?> outermostInputSlot = executionOperator.getOutermostInputSlot(inputSlot);
            if (!this.planEnumeration.requestedInputSlots.contains(outermostInputSlot)) {
                return false;
            }
        }
        return true;
