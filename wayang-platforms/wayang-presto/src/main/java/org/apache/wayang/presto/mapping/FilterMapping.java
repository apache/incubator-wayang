...
import org.apache.wayang.presto.operators.PrestoFilterOperator;
...
/**
 * Maps a {@link FilterOperator} (with a SQL-implementable predicate) to a
 * {@link PrestoFilterOperator}.
 */
...
public class FilterMapping implements Mapping {
    ...
    private ReplacementSubplanFactory createReplacementSubplanFactory() {
        return new ReplacementSubplanFactory.OfSingleOperators<FilterOperator>(
                (matchedOperator, epoch) -> new PrestoFilterOperator(matchedOperator).at(epoch)
        );
    }
}
