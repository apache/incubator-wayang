...
import org.apache.wayang.presto.operators.PrestoGlobalReduceOperator;
...
/**
 * Maps a {@link GlobalReduceOperator} (with a SQL-implementable reduction) to a
 * {@link PrestoGlobalReduceOperator}.
 */
...
public class GlobalReduceMapping implements Mapping {
    ...
    private ReplacementSubplanFactory createReplacementSubplanFactory() {
        return new ReplacementSubplanFactory.OfSingleOperators<GlobalReduceOperator<Record>>(
                (matchedOperator, epoch) -> new PrestoGlobalReduceOperator(matchedOperator).at(epoch)
        );
    }
}
